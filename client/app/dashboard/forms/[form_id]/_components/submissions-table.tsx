...
import { Inbox, Database, Search, Download, BarChart3, Clock, KeySquare, ArrowLeft, ArrowRight, Mail, SendHorizonal, Zap, Rocket, Key } from "lucide-react"
...
export function SubmissionsTable({ projectId, formId }: { projectId: string; formId: string }) {
  ...
  useEffect(() => {
    ...
    api.get(`/api/v1/projects/${projectId}/forms/${formId}/submissions/metadata`)
      .then((res) => {
        if (!mounted) return
  ...
  useEffect(() => {
    ...
    api.get(`/api/v1/projects/${projectId}/forms/${formId}/submissions?size=${size}&page=${page}`)
      .then((res) => {
        if (!mounted) return
  ...
  const downloadCSV = () => {
    ...
    const headers = ["ID", "Received Date", ...visibleColumns]

    const rows = submissions.map(sub => {
      const baseObj = [sub.id, new Date(sub.createdAt).toISOString()]

      const dynamicFields = visibleColumns.map(col => {
      ...
      return [...baseObj, ...dynamicFields].join(",")
    })
    ...
    anchor.setAttribute("download", `form-${formId}-submissions-page-${page + 1}.csv`)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            ...
            <span className="relative z-10 font-medium text-sm tracking-tight text-foreground">Export page CSV</span>
          </button>
        </div>
      </div>
